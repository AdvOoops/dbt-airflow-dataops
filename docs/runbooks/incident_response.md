...
docker logs dbt-airflow-dataops-airflow-scheduler-1 --tail 100

# 2. Check DBT logs
docker exec dbt-airflow-dataops-dbt-1 cat /usr/app/dbt/logs/dbt.log

# 3. Check database connectivity
docker exec dbt-airflow-dataops-dbt-1 dbt debug
...
# 5. Verify fix
docker exec dbt-airflow-dataops-dbt-1 dbt run --select <model_name>
```
...
```bash
# 1. Run specific test
docker exec dbt-airflow-dataops-dbt-1 dbt test --select <model_name>

# 2. Check test results
cat dbt/target/run_results.json | jq '.results[] | select(.status == "fail")'

# 3. Query the data
docker exec -it dbt-airflow-dataops-sqlserver-1 /opt/mssql-tools18/bin/sqlcmd \
  -S localhost -U sa -P "YourStrong@Passw0rd" -C \
  -Q "SELECT TOP 100 * FROM bronze.brnz_customers ORDER BY last_modified_date DESC"

# 4. Check data lineage
docker exec dbt-airflow-dataops-dbt-1 dbt docs generate
...
docker exec dbt-airflow-dataops-dbt-1 dbt run --select <model_name> --debug
```
...
# 3. Verify rollback
docker exec dbt-airflow-dataops-dbt-1 dbt test --target <environment>
```
