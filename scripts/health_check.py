...
class HealthCheck:
    ...
    def check_docker_containers(self):
        ...
        try:
            ...
            running_containers = [c.split('\t')[0] for c in containers if c]
    ...
    def check_airflow_webserver(self):
        ...
        if not HAS_REQUESTS:
            print("  ⚠️  requests library not installed, skipping")
            self.warnings += 1
            return True
    ...
    def check_database_connection(self):
        ...
        if not HAS_PYODBC:
            print("  ⚠️  pyodbc library not installed, skipping")
            self.warnings += 1
            return True
        ...
        try:
            ...
            print(f"     Version: {version.split(chr(10))[0][:50]}...")
    ...
    def check_dbt_models(self):
        ...
        try:
            ...
            models_path = 'dbt/models/' if os.path.exists('dbt/models/') else '../dbt/models/'
            ...
            # Check if models can be compiled via Docker
            result = subprocess.run(
                ['docker', 'exec', 'dbt-airflow-dataops-dbt-1', 'dbt', 'compile', '--project-dir', '/usr/app/dbt'],
                capture_output=True,
                text=True,
                timeout=60
            )

            if 'Completed successfully' in result.stdout or result.returncode == 0:
                print("  ✅ DBT models compile successfully")
                self.checks_passed += 1
                return True
            else:
                print("  ⚠️  DBT compilation has warnings or issues")
                self.warnings += 1
                return True

        except subprocess.TimeoutExpired:
            print("  ⚠️  DBT compilation timed out")
            self.warnings += 1
            return True
        except Exception as e:
            print(f"  ⚠️  Error checking DBT models: {e}")
            self.warnings += 1
            return True
    ...
    def check_recent_pipeline_runs(self):
        ...
        try:
            ...
            target_path = 'dbt/target/run_results.json' if os.path.exists('dbt/target/run_results.json') else '../dbt/target/run_results.json'
