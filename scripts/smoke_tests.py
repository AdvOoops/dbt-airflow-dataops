...
class SmokeTests:
    ...
    def run_all_tests(self):
        ...
        if self.test_table_exists('bronze', 'brnz_customers'):
            tests_passed += 1
        else:
            tests_failed += 1

        # Test 2: Check bronze tables have data
        if self.test_table_has_data('bronze', 'brnz_customers', min_rows=100):
            tests_passed += 1
        else:
            tests_failed += 1

        # Test 3: Check for nulls in primary key
        if self.test_no_nulls_in_column('bronze', 'brnz_customers', 'CustomerID'):
            tests_passed += 1
        else:
            tests_failed += 1

        # Test 4: Check data freshness
        if self.test_data_freshness('bronze', 'brnz_customers', 'last_modified_date', max_age_hours=100000):
            tests_passed += 1
        else:
            tests_failed += 1
